...
アーカイブページは、記事ページと同じスタイルを一部利用しますがレイアウト領域の違いがあります。アーカイブページ用のスタイルは、`.page-archive`クラスを用いて、アーカイブページのスタイルを制御します。共通するスタイルは、`.page-archive`クラスを用いてアーカイブページ用のスタイルを制御します。

アーカイブ全体コンテンツについて

* アーカイブ全体コンテンツは画面の真ん中に配置します。
* 個別のアーカイブ記事を、2列で表示し、左から右の順に並べます。配置にはフレックスボックスを使用します。

個別のアーカイブ記事について

* 個別のアーカイブ記事は、配置の調整がしやすいようにグリッドレイアウトを使用します。
* アーカイブ記事は、サムネイル、記事タイトル、作成日付、カテゴリ名、ソーシャルボタンの順に表示します。記事冒頭部分は表示しません。
* サムネイルは、260px136pxのサイズで表示します。
* 記事タイトルはリンク可能ですが、ホバー時に色変化せず、アンダーラインもつけません。
