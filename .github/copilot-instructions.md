...
* スクリーンショットは、ページ上部、目次、コードハイライトの部分を切り取ってスクリーンショットを取得し、それぞれ適切な紹介セクションで利用します。
...
* はてなブログのテーマストアにはcssのみ含められます。このため、JavaScriptで提供している機能はカスタマイズとして紹介する必要があります。
  * `toc-toggle.html`の設定で、目次開閉機能と次ボタン機能の両方利用できるようになります。別々の機能として紹介するより、まとめて紹介した方が良いでしょう。
