...
* 生成するファイルのインデントは、.editorconfigに沿ってください。
* はてなブログテーマストアで用いる、このテーマの魅力を伝える画像`theme-store.png`を作ってください。
...
コードブロックコピー機能は、記事内のコードブロック（`<pre class="code"><複数のspan></pre>`）に対して、コードを簡単にコピーできるボタンを提供する機能です。

1. コピーボタンのデザインと配置:
   * コードブロックの右上に「コピー」アイコンボタンを配置
   * ボタンは半透明（opacity: 0.7）で控えめに表示し、ホバー時に完全に不透明になる
   * ボタンのサイズはデスクトップでは標準サイズ、モバイルではやや小さめに調整
   * ボタンの背景色はvar(--bg-light)、枠線はvar(--border)で1pxの太さ
   * 角丸は4pxで、全体的に軽い印象に

2. インタラクションとフィードバック:
   * ホバー時に背景色がvar(--bg-hover)に変化
   * フォーカス時に薄い青色のアウトラインを表示（box-shadow利用）
   * コピー成功時にボタンの背景色を緑色(#4caf50)に変更し、テキストを「コピーしました！」に変更
   * 2秒後に元の状態に自動的に戻る

3. コードブロックの調整:
   * コピーボタン用のスペースを確保するため、コードブロックの上部に2.5remのパディングを追加
   * コードブロックに相対位置指定を設定し、ボタンを絶対位置指定でその中に配置

4. レスポンシブ対応:
   * モバイル画面（768px以下）ではボタンのサイズをやや小さく調整（パディング0.2rem 0.5rem、フォントサイズ0.7rem）

この機能は、`scss/lib/_code_copy.scss`でスタイルを定義し、`js/code-copy.js`でJavaScriptの動作を実装しています。ユーザーが設定するときは、`customize-code-copy.html`の内容をはてなブログの「記事上HTML」に貼り付けることで有効になります。
