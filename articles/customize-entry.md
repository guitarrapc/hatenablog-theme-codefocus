...
記事の横幅は`#container`と`#footer`の`max-width`で決まります。既定値は画面幅に応じて3段階です。

| 画面幅 | 記事の横幅 |
| --- | --- |
| 767px以下 | 画面幅いっぱい（左右15pxの余白） |
| 768px以上 | 700px |
| 992px以上 | 790px |
| 1200px以上 | 850px |

デザインCSSはテーマのCSSより後に読み込まれるため、同じセレクタを指定すれば上書きできます。変更するときは`#container`と`#footer`の両方を同じ値にしてください。片方だけ変えると、記事とフッターの左右がずれます。
...
メディアクエリは記述順に上書きされるため、画面幅の小さいものから順に書きます。

#### 幅を広げる

コードブロックや図表を多く載せる場合は、既定より広げたほうが読みやすくなります。既定と同じ3段階を、それぞれ大きい値で上書きします。

```css
@media (min-width: 992px) {
    #container, #footer {
        max-width: 900px; /* 既定は790px */
    }
}

@media (min-width: 1200px) {
    #container, #footer {
        max-width: 1100px; /* 既定は850px */
    }
}
```

横幅1540px以上の画面ではフローティング目次が常時表示されるため、この指定のままだと記事が目次に重なります。次の項目で対策を説明します。

#### フローティング目次と重ならないようにする

フローティング目次は画面の右端に固定され、右端から約305pxを占めます。一方で記事は画面の中央に配置されます。そのため記事を広げると、画面が広いほど記事の右端が目次に近づき、やがて重なります。横幅1600pxの画面で記事を1100pxにすると、記事の右端が目次に62px重なります。

記事の左右に目次のぶんの余白を確保すれば、重なりを避けられます。目次が占める305pxに少し余裕を足して、左右340pxずつを空ける指定です。

```css
@media (min-width: 1200px) {
    #container, #footer {
        /* 上限1100px、ただし左右340pxずつは目次のために空ける */
        max-width: min(1100px, calc(100vw - 680px));
    }
}
```

この指定では、画面幅に応じて記事の幅が次のように決まります。
...
`min()`の第1引数が記事の幅の上限です。ここを変えれば、大画面でどこまで広げるかを調整できます。

#### 目次の常時表示をやめて、さらに広げる

目次の常時表示をやめれば、画面幅を目次に取られずに記事を広げられます。この場合、目次は従来どおり右上の「目次」ボタンから開きます。

まず、「デザイン」→「カスタマイズ」→「ヘッダ」→「ブログタイトル下」に貼り付けた[customize-toc-button.html](https://github.com/guitarrapc/hatenablog-theme-codefocus/blob/main/customize-toc-button.html)の`WIDE_SCREEN_WIDTH`を、常時表示したくない大きな値に変更します。

```js
const CONSTANTS = {
  WIDE_SCREEN_WIDTH: 99999,      // 常時表示を無効にする
```

次にデザインCSSで、横幅1540px以上でも目次ボタンを表示するようにします。テーマは常時表示と入れ替わりでボタンを隠すため、この指定がないとボタンが出てきません。

```css
@media (min-width: 1540px) {
    .toc-button {
        display: inline-flex; /* 大画面でも目次ボタンを表示する */
    }
}
...
**メディアクエリの外に書かない**: `calc(100vw - 680px)`をメディアクエリの外に書くと、スマートフォンでは記事が数百px幅まで潰れます。幅を広げる指定は必ず`min-width`のメディアクエリの中に入れてください。

**画像は元のサイズより大きくならない**: 記事内の画像は`max-width: 100%`で表示されます。記事を広げても、画像は元の解像度を超えて拡大されません。600pxの画像は600pxのまま中央に配置されます。

**コードブロックと表は広がる**: コードブロックと表は記事の幅いっぱいに広がります。幅を広げると、これまで折り返されていた長い行が1行で収まるようになります。

**広げすぎると読みにくい**: 日本語の本文は1行40文字前後が読みやすいとされます。フォントサイズが16pxなら、おおよそ700pxから900pxが目安です。1100pxを超える設定は、コードや図表が主体の記事に向いています。本文主体の記事では読み返しにくくなります。
