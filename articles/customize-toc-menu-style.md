...
[CodeFocusテーマ](https://blog.hatena.ne.jp/-/store/theme/6802418398435013379)の目次は、テーマ本体は変更せずはてなブログの「デザインCSS」に追加するだけでカスタマイズ可能です。
...
本記事で紹介しているカスタマイズスタイルは、記事内目次とフローティング目次の両方に自動的に適用されます。
...
まずは`!important`なしで試し、反映されない箇所だけに追加してください。

以下は、`!important`が必要になることがありえるケースです。
...
CodeFocusテーマは2つの方法でダークモードに対応しています。先に提示したスタイルは、ダークモード対応を含めています。
