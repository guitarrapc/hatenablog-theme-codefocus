...
CodeFocusテーマはプログラミングブログに最適化されていますが、一般的なブログ用途でも使いやすくしています。
...
CodeFocusテーマは、PC、タブレット、スマートフォンなど、複数のデバイスサイズに最適化されており、それぞれのデバイスで読みやすく美しい表示を実現しています。

### デバイス別の記事表示

各デバイスでの表示の違いを示します。PC[^1]では十分な余白を持ったレイアウト、タブレット[^2]ではタッチ操作に適したサイズ調整、スマートフォン[^3]では限られた画面サイズを最大限に活用するレイアウトになっています。
...
記事部分の横幅は3段階で変わります。横幅1200px以上の大きな画面では記事は850pxで表示され、PCや大きなタブレットで快適に読めるようになっています。横幅992pxから1200pxの中程度の画面では記事は790pxで表示され、タブレットや小型ノートPCでも読みやすいサイズです。横幅768px未満のスマートフォンでは記事は700px～100%で表示され、狭い画面でも快適に閲覧できます。いずれもパディングを適切に設定し、コンテンツが画面端に密着しないように配慮されています。
...
はてな標準のコードブロックを使いつつ、プログラミング言語のコードブロックを見やすく表示します。色彩がはっきりしており、長いコードでも読みやすさを狙っています。

Python、C#、Goで同一処理を記述したコードの表示例です。
...
CodeFocusは、アクセシビリティに配慮したデザインを採用しており、GitHubベースのアラート記法を用いた情報提示もサポートしています。
...
ダークモード機能をサポートしています。ダークモードは、ユーザーのシステム設定に応じて自動的に切り替わるほか、手動での切り替えも可能です。

- **システム設定への自動追従**: デフォルトではOSのシステム設定（ライト/ダーク）に合わせて自動的にテーマが切り替わります。
- **手動切り替え**: 画面右上(スマートフォンは右下)に表示されるダークテーマボタンから、ライトモード・ダークモード・システム設定に手動で切り替えることができます。
- **設定の記憶**: 選択したテーマ設定はブラウザのlocalStorageに保存され、次回訪問時も維持されます。

| PC表示 | タブレット表示 | スマートフォン表示 |
| --- | --- | --- |
| [f:id:guitarrapc_tech:20260914165826p:plain:alt=PC表示] | [f:id:guitarrapc_tech:20260914165848p:plain:alt=タブレット表示] | [f:id:guitarrapc_tech:20260914165904p:plain:alt=スマートフォン表示] |
...
5種類のアラートは、それぞれ色・アイコン・タイトルが変わります。
...
| `[!WARNING]` | Warning（黄） | 問題を避けるためにすぐ注意してほしい情報 |
...
記事右上（スマートフォンは右下）に常に表示される目次ボタンを提供します。ボタンをクリックするといつでも目次を表示できる、長い記事でも迷子になりません。

[f:id:guitarrapc_tech:20260212180930p:plain:alt=フローティング目次ボタンのスクリーンショット] <!-- screenshots/pc-toc-button.png -->

目次スタイルはシンプルでありながら使いやすくなるよう工夫しています。
...
- 目次のある記事が画面内にあるときだけ表示され、記事から離れた位置までスクロールすると自動的に消える
- 目次内のリンクをクリックすると自動的に目次が閉じ、記事の該当セクションにスムーズに移動
- 画面外をクリックすると目次が自動的に閉じる

#### 4. ワイドスクリーンでのサイドバイサイド表示

横幅1540px以上の大画面では、目次が画面右側に常時表示され、記事と並んで表示されます。この機能により、以下のような利点があります：

- **目次ボタンは非表示**: 目次が常時表示されているため、ボタンは不要となり画面がすっきり
- **記事と目次を同時に確認**: 長い記事でも、記事を読みながら全体構造を常に把握できる
- **自動的な表示位置調整**: 目次が長い場合、現在読んでいるセクションが自動的に表示範囲内にスクロールされる
- **レスポンシブに対応**: 画面を狭くすると自動的に通常の目次ボタンモードに戻る
...
| PC表示 | タブレット表示 | スマートフォン表示 |
| --- | --- | --- |
| [f:id:guitarrapc_tech:20260212181226p:plain:alt=PC表示の一覧スクリーンショット] | [f:id:guitarrapc_tech:20260212181249p:plain:alt=タブレット表示の一覧スクリーンショット] | [f:id:guitarrapc_tech:20260212181309p:plain:alt=スマートフォン表示の一覧スクリーンショット] |
...
[f:id:guitarrapc_tech:20260914170027p:plain:alt=関連記事表示のスクリーンショット] <!-- screenshots/pc-related-entries.png -->
